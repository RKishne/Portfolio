...
const NavbarIsland = styled(motion.nav)<{ $scrolled: boolean }>`
  ...
  @media (min-width: 769px) {
    margin-top: ${(props) =>
      props.$scrolled ? "var(--spacing-2)" : "var(--spacing-4)"};
    background: ${(props) =>
      props.$scrolled ? "rgba(9, 9, 11, 0.8)" : "rgba(9, 9, 11, 0.5)"};
    width: ${(props) =>
      props.$scrolled
        ? "auto"
        : "100%"}; /* Shrink on scroll? maybe just stay wide */
    min-width: 600px;
  }
...
const NavItem = styled(Link)<{ $active: boolean }>`
  ...
  color: ${(props) =>
    props.$active ? "var(--accent-primary)" : "var(--dark-400)"};
  text-decoration: none;
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            >
              © {new Date().getFullYear()} Rahul kishne
            </motion.div>
          </MobileMenu>
        )}
      </AnimatePresence>
